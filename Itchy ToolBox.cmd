...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$dir=Split-Path -Parent '!dmde_target!'; New-Item -ItemType Directory -Force -Path $dir ^| Out-Null; try { $page=Invoke-WebRequest -Uri 'https://dmde.com/download.html' -UseBasicParsing; $href=($page.Links ^| Where-Object { $_.innerText -match 'GUI for Windows, 64-bit' } ^| Select-Object -First 1).href; if(-not $href){ throw 'DMDE indirme linki bulunamadi' }; if($href -notmatch '^https?://'){ $href='https://dmde.com/'+$href.TrimStart('/') }; Invoke-WebRequest -Uri $href -OutFile '!dmde_target!' -UseBasicParsing; New-Item -ItemType Directory -Force -Path '!dmde_dir!' ^| Out-Null; Expand-Archive -Path '!dmde_target!' -DestinationPath '!dmde_dir!' -Force; $exe=Get-ChildItem '!dmde_dir!' -Recurse -Filter 'dmde.exe' ^| Select-Object -First 1; if($exe){ Start-Process -FilePath $exe.FullName; exit 0 } else { exit 1 } } catch { Write-Host $_.Exception.Message; exit 1 }"
if !errorlevel! == 0 (
